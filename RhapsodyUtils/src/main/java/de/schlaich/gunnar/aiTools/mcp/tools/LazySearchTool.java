 ...
 *
 * Argumente (alle optional au�er query): - query : String (erforderlich) -
 * top_k : int (Default 20) - timeout_ms : long (Default 250) - kinds :
 * Array<String> (z. B. ["Class","Block"]) � Filter auf MetaClass - stereotypes
 * : Array<String> � Filter, wenn Element Stereotypnamen enth�lt - under :
 * String � Qualified-Name-Pr�fix als Suchwurzel (Subbaum) - includeFeatures :
 * boolean � in Class-Features absteigen (Default false) - offset : int � Anzahl
 * bereits akzeptierter Treffer (f�r Pagination)
 *
 * R�ckgabe: { items: [ {id,kind,name,qualifiedName,stereotype} ... ],
 * truncated: boolean, // true, wenn per timeout/Limit abgebrochen visited: int,
 * // besuchte Knoten next_offset: int // offset + items.length (f�r naive
 * Pagination) }
 */
...
//		{
//			// wenn Filter, aber kein Startknoten, dann auf Top-Level beschr�nken
