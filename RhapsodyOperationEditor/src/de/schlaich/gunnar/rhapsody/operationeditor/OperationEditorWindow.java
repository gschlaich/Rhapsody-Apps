...
public class OperationEditorWindow extends JRootPane implements HyperlinkListener, ActionListener, SyntaxConstants
{
			...
			boolean aExitOnClose)
	{
		...
		myApplicationListener = new ApplicationListener(myFrame, mySelectedOperation, myApplication, myTextArea,
				myStartAutoCompletion, myExitOnClose);
			...
			public void windowClosing(WindowEvent e)
			{
				if (textChanged())
				{
					int n = JOptionPane.showConfirmDialog(null,
							...
							"Apply changes to model?", JOptionPane.OK_CANCEL_OPTION);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		try
		{
			...
			if (mySelectedOperation.isReadOnly() == 0)
			{
				if (command.equals("ok") || command.equals("apply") || command.equals("generate")
						|| command.equals("compile") || command.equals("build"))
				{
					String text = myTextArea.getText();
					setActionbody(text);
					if (mySelectedOperation != null)
			...
			if (command.equals("cancel"))
			{
				...
				{
					int n = JOptionPane.showConfirmDialog(null, "Ok: Changes are deleted\nCancel: Return to editor",
							"Discard all changes in the editor?", JOptionPane.OK_CANCEL_OPTION);
			...
			if (command.equals("roundTrip"))
			{
				if (textChanged())
				{
					int n = JOptionPane.showConfirmDialog(null,
							...
							"Roundtrip from source file?", JOptionPane.YES_NO_OPTION);
			...
			if (command.equals("revert"))
			{
				...
				{
					int n = JOptionPane.showConfirmDialog(null,
							"Yes will overwrite changes in editor\nNo: Cancel revert", "Revert from model?",
		...
		catch(Exception e1)
		{
			...
			JOptionPane.showMessageDialog(null, "Exception occured: " + e1.getMessage(), "Changes are deleted",
...
class OpenFeature extends TextAction
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		LocalCompletionProvider localCompletionProvider = new LocalCompletionProvider(tc.getText(),
		...
		if (myCompletionProvider != null)
		{
			...
			if (c == null)
			{
				...
				{
					IASTNode node = ASTHelper.getNodeAtPosition(tc.getSelectionStart(), tc.getSelectionEnd(),
							translationUnit);
					if (node != null)
					{
	...
	private boolean openOperationFeature(String aElementName, LocalCompletionProvider aLocalCompletionProvider,
			IASTNode aNode)
	{
		Completion c;
	...
	/**
	 * Gets the filename that the caret is sitting on. Note that this is a somewhat
	 * naive implementation and assumes filenames do not contain whitespace or other
	 * "funny" characters, but it will catch most common filenames.
	 * 
