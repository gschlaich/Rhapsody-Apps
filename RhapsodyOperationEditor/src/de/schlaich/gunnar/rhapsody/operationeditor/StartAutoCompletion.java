...
public class StartAutoCompletion extends Thread
{
	...
	public StartAutoCompletion(RSyntaxTextArea aTextArea, AutoCompletion aAutoComplete, IRPOperation aSelectedOperation,
			IRPApplication aApplication, JFrame aFrame)
	{
	...
	/**
	 * Creates the completion provider for a C editor. This provider can be shared
	...
	private CompletionProvider createCompletionProvider(IRPOperation selectedOperation)
	{
		...
		myClassifierCompletionProvider = ClassifierCompletionProvider.GetProvider(selectedClass, visibility.v_private,
				true);
		// LocalCompletionProvider localCP = new
		// LocalCompletionProvider(selectedOperation.getBody());
	...
	private void createTraceCompletions(ClassifierCompletionProvider aCp)
	{
		...
		aCp.addCompletion(new BasicCompletion(aCp, "reinterpret_cast",
	...
	private void getNameSpaces(DefaultCompletionProvider provider, IRPModelElement selected)
	{
		...
		IRPPackage packageStd = (IRPPackage) mySelectedOperation.getProject().findNestedElementRecursive("std",
				"Package");
		if (packageStd != null)
		{
		...
		IRPPackage packageOssi = (IRPPackage) mySelectedOperation.getProject().findNestedElementRecursive("OSSI",
				"Package");
		if (packageOssi != null)
		{
			RhapsodyNamespaceCompletion rnc = new RhapsodyNamespaceCompletion(provider, packageStd);
