...
public class PlantUMLGenerator
{
	...
	public PlantUMLGenerator(IRPModelElement aIRPElement, boolean aGenerateInheritanceHierarchy)
	{
		...
		if (aGenerateInheritanceHierarchy)
		{
			if (aIRPElement instanceof IRPClass)
			{
				...
				plantStringBuffer.append("\n");
				plantStringBuffer.append(PlantUMLElements.mySetSeparator);
				plantStringBuffer.append("\n");
	...
	private void generateInheritanceHierarchy(IRPClassifier aIRPClass, StringBuffer aPlantStringBuffer,
		...
		for (IRPModelElement element : elements)
		{
			...
			if (element instanceof IRPOperation)
			{
				...
				if (owner instanceof IRPClassifier)
				{
					...
					aPlantStringBuffer.append(PlantUMLElements.myClass);
					aPlantStringBuffer.append(namespace);
					aPlantStringBuffer.append(PlantUMLElements.mySeparator);
					aPlantStringBuffer.append(owner.getName());
					aPlantStringBuffer.append(PlantUMLElements.myBraceOpen);

					if (operation.getIsStatic() == 1)
					{
						aPlantStringBuffer.append("{static} ");
					}
					if (operation.getIsAbstract() == 1)
					{
						aPlantStringBuffer.append("{abstract} ");
					}
					...
					addStereotype(aPlantStringBuffer, operation);

					if (operation.getIsDtor() == 1)
					{
						aPlantStringBuffer.append("\\");
					}
					aPlantStringBuffer.append(operation.getSignature());
					if (operation.getReturns() != null)
					{
						aPlantStringBuffer.append(":");
						aPlantStringBuffer.append(operation.getReturns().getName());
					}
					aPlantStringBuffer.append(PlantUMLElements.myBraceClose);
				}
			}
			if (element instanceof IRPAttribute)
			{
				...
				if (owner instanceof IRPClassifier)
				{
					...
					aPlantStringBuffer.append(attribute.getName());
					aPlantStringBuffer.append(":");
					aPlantStringBuffer.append(attribute.getType().getName());
					addStereotype(aPlantStringBuffer, attribute);
					aPlantStringBuffer.append(PlantUMLElements.myBraceClose);
				}
			}
	...
	private void addStereotype(StringBuffer aPlantStringBuffer, IRPModelElement element)
	{
		List<IRPStereotype> stereotypes = element.getStereotypes().toList();
		if (stereotypes.size() > 0)
		{
			aPlantStringBuffer.append(PlantUMLElements.myStereotypeOpen);

			for (IRPStereotype stereotype : stereotypes)
			{
				aPlantStringBuffer.append(stereotype.getName());
			...
			aPlantStringBuffer.append(PlantUMLElements.myStereotypeClose);
		}
	}
	...
	private boolean generate(IRPClassifier aClassifier, StringBuffer aPlantStringBuffer, long aLevel)
	{
		...
		addStereotype(aPlantStringBuffer, aClassifier);
		...
		if (aLevel >= 1)
		{
			 ...
			 * for(IRPModelElement element:elements) { generateElement(element,
			 * aPlantStringBuffer, 0); }
			 */
	...
	private void generate(IRPDependency aDependency, StringBuffer aPlantStringBuffer, long aLevel)
	{
		...
		{
			aPlantStringBuffer.append(" \"");
			addStereotype(aPlantStringBuffer, aDependency);
...
class PlantUMLElements
{
	...
	public static String mySetSeparator = "set separator ::";
